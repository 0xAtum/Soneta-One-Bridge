...
abstract contract BaseScript is Script {
  ...
  function _loadChains() private {
    ChainConfig[] memory chainConfigs = abi.decode(vm.parseJson(_getConfig(CHAINS_CONFIG_FILE_NAME)), (ChainConfig[]));
    ...
    for (uint256 i = 0; i < chainConfigs.length; ++i) {
      ...
      chainMetadata[indexChainConfig.chainId] = ChainMetadata(indexChainConfig.name, indexChainConfig.chainId, false);
  ...
  function _loadDeployedContracts(bool _loadInSimulation) private {
    ...
    for (uint256 i = 0; i < length; ++i) {
      ...
      contracts[_contractNameToBytes32(_getNetwork(), cached.contractName)] = cached.contractAddress;
   ...
   */
  function _loadOtherContractNetwork(bool _loadInSimulation, string memory _network) internal {
    if (!_loadInSimulation && _isSimulation()) return;
    ...
    for (uint256 i = 0; i < length; ++i) {
      cached = deployments[i];
      contracts[_contractNameToBytes32(_network, cached.contractName)] = cached.contractAddress;
      vm.label(cached.contractAddress, cached.contractName);
    }
  }
   ...
   */
  function _getDeployedContracts(string memory _network) internal returns (Deployment[] memory deployments_) {
   ...
   */
  function _getDeployedContractsJson(string memory _network) private returns (bytes memory jsonBytes_) {
  ...
  function _contractNameToBytes32(string memory _network, string memory _contractName) internal pure returns (bytes32) {
    return keccak256(abi.encode(_network, _contractName));
  }

  function _tryGetContractAddress(string memory _contractName) internal view returns (address) {
    return contracts[_contractNameToBytes32(_getNetwork(), _contractName)];
  }

  function _tryGetContractAddress(string memory _network, string memory _contractName) internal view returns (address) {
